...
/*
 * Physical roll, pitch and yaw tests established the component
 * signs used by the current body-frame conversion.
 *
 * Apply the same mapping to both accelerometer and gyroscope
 * vectors so estimator attitude and measured body rates use the
 * same convention:
 *
 *     sensor X -> body +X
 *
 ...
 *             positive
 *
 *         attitude estimator yaw
 *             positive
 *
 *
 * Therefore the finalized controller-side yaw convention is:
 *
 *     +yaw = nose turns right
 */
    ...
    imu_body_sample_t *body)
{
    ...
    *body =
    ...
    body->sample_interval_us =
        gyro->sample_interval_us;
    ...
    body->specific_force_x_g =
        SENSOR_TO_BODY_X_SIGN *
        accel->filtered_x_g;


    body->specific_force_y_g =
        SENSOR_TO_BODY_Y_SIGN *
        accel->filtered_y_g;


    body->specific_force_z_g =
        SENSOR_TO_BODY_Z_SIGN *
        accel->filtered_z_g;
